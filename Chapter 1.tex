...
Options with ``special or unusual features'' are called ``exotic options''. The exotic options whose pricing is supported by \mintinline{R}|rmcop| package is introduced below based on the definitions given in \textit{An Introduction to Financial Option Valuation} \cite{Higham2004}.
...
Recall the explicit form of an SDE $f(S_t)$ given by the Itô-Doeblin rule in Equation \ref{eq:GBM_explicit}. We assign $f(S_t)=\log(S_t)$ where $\log$ is the natural logarithm. The solution is:
